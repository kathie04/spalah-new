...
gulp.task('html', function () {
  ...
  return gulp.src(paths.src + '/*.{html,pug}')
    .pipe(plumber({
        errorHandler: notify.onError("Error: <%= error.message %>")
    }))
    .pipe(pug({pretty: true}, options))
...
gulp.task('less', function() {
    return gulp.src(paths.src + paths.less + '/**/styles.less')
        .pipe(plumber({
            errorHandler: notify.onError("Error: <%= error.message %>")
        }))
        .pipe(gulpif(isDebug, sourcemaps.init()))
...
gulp.task('fonts', function() {
    return gulp.src(paths.src + paths.fonts + '//**/*.{eot,ttf,woff}')
...
gulp.task('watch', function() {
    watch([paths.src + '/**/*.{html,pug}', './src/data/data.json'], function() {
        gulp.start('html');
    });
    ...
    watch(paths.src + paths.fonts + '/**/*.{eot,ttf,woff}', function() {
        gulp.start('fonts');
    });
});
